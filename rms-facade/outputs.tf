...
locals {
    ...
    network_output = module.oci_lz_orchestrator.network_resources != null ? {
        "network_resources" : {
            ...
            "dns_resolver" : {for k, v in module.oci_lz_orchestrator.network_resources.dns_resolver : k => {"id" : try(v.id,null)}}
    ...
    nlbs_output = length(module.oci_lz_orchestrator.nlb_resources.nlbs_private_ips) > 0 ? {
        "nlbs_private_ips" : {for k, v in module.oci_lz_orchestrator.nlb_resources.nlbs_private_ips : k => {"id" : try(v.private_ips[0].id,null)}},
